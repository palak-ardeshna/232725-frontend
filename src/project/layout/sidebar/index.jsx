...
const hasPermission = (userType, permissions, module) => {
...
const DashboardSidebar = ({ collapsed, isMobile, onBackClick }) => {
    ...
    const filteredCrmMenuItems = crmMenuItems.filter(item => {
    ...
    if (filteredCrmMenuItems.length > 0) {
        if (collapsed) {
            ...
            menuItems.push({
                ...
                icon: <RiMessage2Fill />,
                label: 'CRM',
                children: filteredCrmMenuItems.map(item => ({
                    key: item.key,
                    icon: item.icon,
                    label: item.label,
                    className: !hasDashboardAccess ? 'disabled-menu-item' : item.className || ''
    ...
    if (filteredHrmMenuItems.length > 0) {
        if (collapsed) {
            menuItems.push({
                key: 'hrm-collapsed',
                icon: (
                    <Popover
                        content={hrmPopoverContent}
                        trigger="click"
                        placement="right"
                        open={hrmPopoverVisible}
                        onOpenChange={(visible) => {
                            setHrmPopoverVisible(visible);
                        }}
                        overlayClassName="crm-sidebar-popover"
                        align={{
                            offset: [10, 0]
                    ...
                    >
                        <div onClick={(e) => {
                            if (!hasDashboardAccess) return;
                            e.stopPropagation();
                            setHrmPopoverVisible(!hrmPopoverVisible);
                        }}>
                            <RiGroupLine />
                        </div>
                    </Popover>
            ...
            menuItems.push({
                ...
                icon: <RiGroupLine />,
                label: 'HRM',
                children: filteredHrmMenuItems.map(item => ({
                    key: item.key,
                    icon: item.icon,
                    label: item.label,
                    className: !hasDashboardAccess ? 'disabled-menu-item' : item.className || ''
    ...
    if (filteredSettingsMenuItems.length > 0) { 
        if (collapsed) {
            menuItems.push({
                ...
                icon: (
                    <Popover
                        ...
                        onOpenChange={(visible) => {
                            setSettingsPopoverVisible(visible);
                        }}
                        overlayClassName="crm-sidebar-popover"
                        align={{
                            offset: [10, 0]
                    ...
                    >
                        <div onClick={(e) => {
                            e.stopPropagation();
                            setSettingsPopoverVisible(!settingsPopoverVisible);
                        }}>
                            <RiSettings3Line />
                        </div>
                    </Popover>
            ...
            menuItems.push({
                ...
                icon: <RiSettings3Line />,
                label: 'Settings',
                children: filteredSettingsMenuItems.map(item => ({
                    key: item.key,
                    icon: item.icon,
                    label: item.label
                }))
