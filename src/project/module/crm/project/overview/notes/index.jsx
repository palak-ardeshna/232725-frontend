...
import '../../../lead/overview/notes/notes.scss';

const NotesTab = ({ project, customTitle, noteType = 'project', showOnlyUserNotes = true }) => {
    ...
    const { data: singleNoteData, isLoading: isLoadingSingleNote } = useGetNoteQuery(
        editingNoteId,
        { skip: !editingNoteId }
    );
    ...
    useEffect(() => {
        if (singleNoteData && singleNoteData.data) {

            const note = singleNoteData.data;

            const formattedNote = {
                id: note.id,
                noteTitle: note.noteTitle || "",
                description: note.description || "",
                priority: note.priority || "general"
            };
    ...
    const handleAdd = () => {
        if (!project?.id) {
            message.error('Cannot add note: Project information is missing');
            return;
        }
    ...
    const handleRemoveAll = async () => {
        try {
            Modal.confirm({
                ...
                content: 'Are you sure you want to delete all notes for this project? This action cannot be undone.',
                okText: 'Delete All',
                okType: 'danger',
                cancelText: 'Cancel',
                onOk: async () => {
    ...
    const moduleTitle = customTitle || 'Project Notes';
        ...
        <div className="project-notes-tab">
            ...
            >
                ...
                >
                    <NoteForm
                        key={formKey}
                        isSubmitting={isCreating || isUpdating || isLoadingSingleNote}
