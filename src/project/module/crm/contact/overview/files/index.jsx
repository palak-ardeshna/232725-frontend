...
import { useUpdateContactMutation, useGetContactQuery } from '../../../../../../config/api/apiServices';
...
const FilesTab = ({ contact }) => {
    ...
    const { data: contactData, isLoading: isLoadingContact, refetch } = useGetContactQuery(contact?.id, {
        skip: !contact?.id,
    ...
    useEffect(() => {
        let contactFiles = [];
        const contactFilesData = contactData?.data?.files || contact?.files;

        if (contactFilesData) {
            if (typeof contactFilesData === 'string') {
                try {
                    contactFiles = JSON.parse(contactFilesData);
                } catch (e) {
                    console.error('Error parsing files:', e);
                    contactFiles = [];
                }
            }
            else if (Array.isArray(contactFilesData)) {
                contactFiles = contactFilesData;
            }
        }

        setFiles(Array.isArray(contactFiles) ? contactFiles : []);
    }, [contactData, contact]);
    ...
    const handleRemoveAll = async () => {
        try {
            Modal.confirm({
                ...
                content: 'Are you sure you want to delete all files from this contact? This action cannot be undone.',
                okText: 'Delete All',
                okType: 'danger',
                cancelText: 'Cancel',
                onOk: async () => {
    ...
    const isLoading = contact?.id ? isLoadingContact : false;
    ...
    return (
        <div className="contact-files-tab">
            <ModuleLayout
                title="Contact Files"
                icon={<FiFile />}
